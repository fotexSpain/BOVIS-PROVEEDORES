 ...
 ******************************************/
	...
	class PluginComproveedoresSeguro extends CommonDBTM{

		static $rightname	= "plugin_comproveedores";

		static function getTypeName($nb=0){
			return _n('Seguro','Seguros',1,'comproveedores');
		}

		function getTabNameForItem(CommonGLPI $item, $tabnum=1,$withtemplate=0){
			if($item-> getType()=="Supplier"){
				return self::createTabEntry('Seguros de la empresa');
			}
			return 'Seguros';
		}
		...
		static function displayTabContentForItem(CommonGLPI $item,$tabnum=1,$withtemplate=0){
			...
			if($item->getType()=='Supplier'){
				...
				if(isset($item->fields['cv_id'])){
					$self->showFormSeguros($item, $withtemplate);
				}else{
    ...
    // Only allowed types
		...
		function showFormSeguros($item, $withtemplate='') {	
